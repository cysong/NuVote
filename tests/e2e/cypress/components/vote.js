...
export function disable_vote(competition_id) {
    ...
    cy.get("#dailyVotesChart").should("be.visible").scrollIntoView({ duration: 500 });
    cy.wait(1000);
    cy.get("#competitorsLineChart").should("be.visible").scrollIntoView({ duration: 500 });
    cy.wait(1000);
    cy.get("h2").contains("Votes by Ip").scrollIntoView({ duration: 500 });
    cy.wait(1000);
    ...
    cy.wait(500);
    cy.get("#confirmAbandonByIdModal .btn").contains("Confirm").click();
    ...
    cy.get("#status-filter").uncheck().should("not.be.checked");
